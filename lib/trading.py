...
def get_hist_data(code=None, start=None, end=None,
    ...
    if ktype.upper() in ct.K_LABELS:
        url = ct.DAY_PRICE_URL%(ct.K_TYPE[ktype.upper()], symbol)
    elif ktype in ct.K_MIN_LABELS:
        url = ct.DAY_PRICE_MIN_URL%(symbol, ktype)
    else:
    ...
    for _ in range(retry_count):
        ...
        try:
            request = Request(url)
            request.add_header("User-Agent", ct.USER_AGENT)
            lines = urlopen(request, timeout = ct.API_TIMEOUT).read()
...
def get_tick_data(code=None, date=None, retry_count=3, pause=0.001):
    ...
    for _ in range(retry_count):
        time.sleep(pause)
        try:
            re = Request(ct.TICK_PRICE_URL % (date, symbol))
            lines = urlopen(re, timeout=ct.API_TIMEOUT).read()
...
def get_realtime_quotes(symbols=None):
    ...
    request = Request(ct.LIVE_DATA_URL%(_random(), symbols_list))
    text = urlopen(request,timeout=ct.API_TIMEOUT).read()
...
def get_index():
    ...
    -------
      DataFrame
          ...
          amount:成交金额（亿元）
    """
    request = Request(ct.INDEX_HQ_URL)
    text = urlopen(request, timeout=10).read()
    text = text.decode('GBK')
...
def _get_index_url(index, code, qt):
    if index:
        url = ct.HIST_INDEX_URL%(code, qt[0], qt[1])
    else:
        url = ct.HIST_FQ_URL%(code, qt[0], qt[1])
    return url
...
if __name__ == '__main__':
    data = get_h_data('002337',start='2015-01-01',end='2015-03-16')
    print data
